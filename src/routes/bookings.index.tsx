...
type SortKey = "NEW" | "OLD" | "PRICE_DESC" | "PRICE_ASC" | "RECENT";
...
function filterByTab(list: BookingDto[], tab: TabKey): BookingDto[] {
  const d = new Date();
...
function sortBookings(list: BookingDto[], sort: SortKey): BookingDto[] {
  const arr = [...list];
  switch (sort) {
    case "NEW":
          ...
          {isLoading ? (
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="h-[104px] animate-pulse rounded-2xl bg-white/70" />
              ))}
            </div>
          ) : (
            <BookingStats bookings={all} />
          )}
        </section>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="relative">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl border border-border/60 bg-white px-3.5 py-2.5 text-[12.5px] text-ink shadow-sm transition-colors hover:border-primary/40"
                >
                ...
                <AnimatePresence>
                  {sortOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -4 }}
                      className="absolute right-0 top-full z-20 mt-2 w-56 overflow-hidden rounded-xl border border-border/60 bg-white p-1 shadow-lg"
                    >
              ...
              {isLoading && (
                <>
                  {Array.from({ length: 2 }).map((_, i) => (
                    <div key={i} className="h-[260px] animate-pulse rounded-2xl bg-white/70" />
                  ))}
                </>
              )}
                  ...
                  <div className="rounded-2xl border border-border/60 bg-white p-12 text-center">
                    ...
                    {info.showCtas && (
                      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
                        <Link
                          to="/search"
                          className="inline-flex items-center rounded-xl bg-primary px-4 py-2.5 text-[12.5px] font-medium text-primary-foreground hover:bg-primary-mid"
                        >
                          Explore Stays
                        </Link>
                        <button
              ...
              <div className="mt-8 flex flex-col items-center gap-3">
                <div className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-white p-1 shadow-sm">
                  <button
                    ...
                    className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-primary/[0.06] hover:text-primary disabled:opacity-40 disabled:hover:bg-transparent"
                  ...
                  {Array.from({ length: totalPages }).map((_, i) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => setPage(n)}
                        className={`min-w-9 rounded-full px-3 py-1.5 text-[12.5px] transition-colors ${
                          active
