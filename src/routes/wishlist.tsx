...
export const Route = createFileRoute("/wishlist")({
  head: () => ({
    meta: [
      { title: "My Wishlist — StayNest" },
      {
        name: "description",
        content:
          "Your saved stays on StayNest. Revisit bookmarked hotels and plan your next getaway.",
      },
      { property: "og:title", content: "My Wishlist — StayNest" },
      {
        property: "og:description",
        content: "Your saved stays on StayNest.",
      },
    ],
  }),
        ...
        <header className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 flex items-center gap-3 font-display text-4xl leading-tight text-primary md:text-5xl">
              <Heart
                className="h-8 w-8 fill-primary/10 text-primary"
                strokeWidth={1.6}
                aria-hidden
              />
              My Wishlist
          ...
          <Link
            to="/search"
            className="inline-flex items-center gap-2 self-start rounded-xl bg-primary px-5 py-3 text-[13px] font-medium text-primary-foreground shadow-sm transition-all hover:bg-primary-mid"
          >
        ...
        <section className="mt-10">
          {!isAuthenticated && (
            <EmptyShell
              ...
              body="Once signed in, every stay you bookmark shows up here."
          ...
          {isAuthenticated && !isLoading && !isError && hotels.length === 0 && (
            <EmptyShell
              title="Your wishlist is empty"
              body="Start exploring stays to save them! Tap the heart on any hotel card."
              cta={{ to: "/search", label: "Explore Stays" }}
            />
          )}
...
function EmptyShell({
  ...
  return (
    <div className="rounded-3xl border border-border/60 bg-white p-12 text-center">
      ...
      <Link
        to={cta.to}
        className="mt-6 inline-flex items-center rounded-xl bg-primary px-5 py-2.5 text-[12.5px] font-medium text-primary-foreground hover:bg-primary-mid"
      >
        {cta.label}
      </Link>
    </div>
