...
export const Route = createFileRoute("/checkout/$bookingId")({
  head: () => ({
    meta: [
      { title: "Checkout — StayNest" },
      { name: "description", content: "Add companions and confirm your booking with StayNest." },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-[1200px] px-4 pt-24 pb-16 md:px-8">
        <button
          type="button"
          onClick={() => navigate({ to: "/hotels/$hotelId", params: { hotelId: String(booking.hotel.id) } })}
          ...
          <section className="space-y-6">
            {step === "guests" ? (
              <GuestsStep
                ...
                onContinue={() => addGuestsM.mutate(selectedIds)}
                canContinue={selectedIds.length > 0 && !addGuestsM.isPending}
                pending={addGuestsM.isPending}
              />
            ) : (
              <ReviewStep
                booking={booking}
        ...
        return (
          <div key={it.key} className="flex items-center gap-3">
            <div className={[
              ...
              done ? "bg-primary text-primary-foreground" : active ? "border border-primary/50 text-primary" : "border border-border/70 text-muted-foreground",
            ].join(" ")}>
              {done ? <CheckCircle2 className="h-3.5 w-3.5" /> : i + 1}
            </div>
            <span className={active ? "text-[13px] text-primary" : "text-[13px] text-muted-foreground"}>{it.label}</span>
...
function GuestsStep({
  guests, selectedIds, onToggle, onAdd, onContinue, canContinue, pending,
}: {
  guests: GuestDto[];
  selectedIds: number[];
...
function ReviewStep({
  booking, guests, onEditGuests, onPay, paying,
}: {
        ...
        <div className="mt-5 flex gap-4">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 inline-flex items-center gap-1.5 text-[12.5px] text-muted-foreground">
              <MapPin className="h-3.5 w-3.5" /> {booking.hotel.city}{booking.hotel.country ? `, ${booking.hotel.country}` : ""}
            </div>
      ...
      <div className="rounded-2xl border border-border/70 bg-background p-6 md:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-muted-foreground">{guests.length} companion{guests.length === 1 ? "" : "s"}</p>
          </div>
          ...
          {guests.map((g) => (
            <li key={g.id} className="flex items-center gap-3 py-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[12px] text-muted-foreground">{g.gender?.toLowerCase()} · DOB {g.dateOfBirth}</div>
              </div>
            </li>
          ))}
        </ul>
...
function Summary({ booking, nights }: { booking: Awaited<ReturnType<typeof getBooking>>; nights: number }) {
    ...
    <div className="rounded-2xl border border-border/70 bg-background/95 p-6 shadow-[0_20px_50px_-24px_rgba(26,46,32,0.25)]">
      ...
      <div className="mt-4 space-y-2 text-[13px] text-muted-foreground">
        <div className="flex justify-between">
          <span>{currency.format(nightly)} × {nights} night{nights === 1 ? "" : "s"}</span>
      ...
      <Link
        to="/bookings"
        className="mt-4 inline-flex items-center gap-1.5 text-[12px] text-muted-foreground hover:text-primary"
      >
