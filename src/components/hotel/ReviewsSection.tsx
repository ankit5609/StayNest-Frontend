...
export function ReviewsSection({ hotelId, averageRating, reviewCount }: { hotelId: string; averageRating: number; reviewCount: number }) {
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 inline-flex items-center gap-2 text-[13.5px] text-muted-foreground">
            ...
            · Based on {reviewCount} verified stays
          </p>
        </div>
      </div>
            ...
            <article key={r.id} className="rounded-2xl border border-border/70 bg-background/70 p-5">
              <header className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[14px] font-medium text-ink">{r.userName}</div>
              ...
              <p className="mt-3 text-[13.5px] leading-relaxed text-ink/80">{r.reviewText}</p>
            </article>
          ))}
        </div>
      )}
      ...
      {data && data.totalPages > 1 && (
        <div className="flex justify-center pt-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-border px-5 py-2 text-[12.5px] font-medium text-ink transition hover:border-primary/40 hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
          >
