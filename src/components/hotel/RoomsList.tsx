...
interface Props {
  ...
  onSelect: (room: RoomInfoDto) => void;
}

export function RoomsList({ rooms, nights, selectedRoomId, onSelect }: Props) {
  return (
...
function RoomCard({ room, nights, selected, onSelect }: { room: RoomInfoDto; nights: number; selected: boolean; onSelect: () => void }) {
  const photos = room.photos?.length ? room.photos : ["/placeholder.svg"];
  ...
  return (
    <article
      ...
      onMouseLeave={() => { setHover(false); setI(0); }}
      className={[
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-muted">
        ...
        />
        {selected && (
          <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-primary px-2.5 py-1 text-[11px] font-medium text-primary-foreground">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        {room.amenities?.length > 0 && (
          <ul className="flex flex-wrap gap-x-4 gap-y-1 text-[12.5px] text-muted-foreground">
