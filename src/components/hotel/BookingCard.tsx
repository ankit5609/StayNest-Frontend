...
export function BookingCard({ room, startDate, endDate, roomsCount, nights, onReserve }: Props) {
  const total = room ? room.price : 0;
  const taxes = Math.round(total * 0.12);
  const grand = total + taxes;

  return (
    <aside className="sticky top-24 space-y-4 rounded-2xl border border-border/70 bg-background/95 p-6 shadow-[0_20px_50px_-24px_rgba(26,46,32,0.25)] backdrop-blur">
      <div>
        ...
        {room ? (
          <p className="mt-1 text-[12.5px] text-muted-foreground">{room.type}</p>
        ) : (
          <p className="mt-1 text-[12.5px] text-muted-foreground">Select a room to see the total</p>
        )}
      </div>
      ...
      <button
        ...
        onClick={onReserve}
        disabled={!room}
        className="w-full rounded-full bg-primary py-3 text-[13.5px] font-medium text-primary-foreground shadow-sm transition hover:bg-primary-mid disabled:cursor-not-allowed disabled:opacity-50"
      >
