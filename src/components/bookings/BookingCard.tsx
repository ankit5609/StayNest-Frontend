...
function fmtBookingId(id: number) {
  return `#BK${id}`;
}

function guestSummary(b: BookingDto) {
    ...
    >
      <div className="flex flex-col gap-0 md:flex-row">
        <Link
          ...
          className="relative block h-52 w-full shrink-0 overflow-hidden md:h-auto md:w-[240px]"
        >
          <img
            src={photo}
        ...
        <div className="flex flex-1 flex-col p-5">
          <header className="flex items-start justify-between gap-4">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10.5px] font-semibold tracking-wide ${status.bg} ${status.text}`}
              >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    className="grid h-8 w-8 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-primary/[0.06] hover:text-primary"
                  >
                ...
                <DropdownMenuContent align="end" className="w-52">
                  <DropdownMenuItem asChild>
                    <Link to="/bookings/$bookingId" params={{ bookingId: String(booking.id) }} className="cursor-pointer">
            ...
            <div className="flex flex-wrap items-end justify-between gap-4">
              ...
              <div className="flex items-end gap-4">
                ...
                <Link
                  to="/bookings/$bookingId"
                  params={{ bookingId: String(booking.id) }}
                  className="inline-flex items-center rounded-xl bg-primary px-4 py-2.5 text-[12.5px] font-medium text-primary-foreground shadow-sm transition-all duration-200 hover:bg-primary-mid hover:shadow-[0_8px_20px_-10px_rgba(17,26,19,0.4)]"
                >
                  {primaryCta}
                </Link>
              </div>
