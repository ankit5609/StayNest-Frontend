...
import type { MyReviewDto } from "@/lib/api/reviews";
...
function ReviewCard({ review }: { review: MyReviewDto }) {
  return (
    <article className="group flex gap-4 rounded-2xl border border-border/60 bg-background/60 p-4 transition-all hover:-translate-y-[1px] hover:border-primary/30 hover:shadow-[0_10px_28px_-18px_rgba(17,26,19,0.35)] md:p-5">
      {review.hotelPhoto && (
        <img
          src={review.hotelPhoto}
          alt={review.hotelName ?? "Hotel"}
          className="hidden h-20 w-20 shrink-0 rounded-xl object-cover sm:block md:h-24 md:w-24"
          loading="lazy"
        />
      )}
...
export function MyReviewsSection() {
  ...
  return (
    <section className="rounded-3xl border border-border/60 bg-white p-6 shadow-[0_20px_60px_-40px_rgba(17,26,19,0.35)] md:p-8">
      ...
      {isLoading ? (
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-border/70 bg-background/60 p-6 text-center">
          <p className="text-[13.5px] text-ink">Unable to load your reviews.</p>
          <button
            onClick={() => refetch()}
            className="mt-3 inline-flex items-center rounded-xl bg-primary px-4 py-2 text-[12px] font-medium text-primary-foreground hover:bg-primary-mid"
          >
            Retry
          </button>
        </div>
      ) : reviews.length === 0 ? (
        <div className="mt-5 rounded-2xl border border-dashed border-border/70 bg-background/60 p-8 text-center">
            ...
            <div className="mt-6 flex items-center justify-between text-[12.5px] text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={data.first}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-border/70 text-primary transition-colors hover:border-primary/40 disabled:cursor-not-allowed disabled:opacity-40"
