...
import type { Gender, UserDto } from "@/lib/api/users";
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      id="edit-dob"
                      variant="outline"
                      className={`w-full h-11 justify-start text-left font-normal rounded-xl border border-border/70 bg-white px-3 shadow-none ${
                        !dob && "text-muted-foreground/60"
                      }`}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4 text-muted-foreground/70" />
                      {dob ? (
                        format(parseDate(dob)!, "PPP")
                      ) : (
                  ...
                  <PopoverContent className="w-auto p-0 rounded-2xl border-border/70" align="start">
                    <Calendar
                      ...
                      onSelect={(date) => setDob(formatDateStr(date))}
