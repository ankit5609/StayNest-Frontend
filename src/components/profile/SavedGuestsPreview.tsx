...
export function SavedGuestsPreview() {
  ...
  const guests: GuestDto[] = data?.content ?? [];
  ...
  return (
    <section className="rounded-3xl border border-border/60 bg-white p-6 shadow-[0_20px_60px_-40px_rgba(17,26,19,0.35)] md:p-8">
      <div className="flex flex-col gap-4 border-b border-border/60 pb-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          to="/guests"
          className="group inline-flex items-center gap-1.5 self-start text-[13px] font-medium text-primary transition-colors hover:text-primary-mid"
        >
          Manage Guests
      ...
      {isLoading ? (
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-border/70 bg-background/60 p-6 text-center">
          ...
          <Link
            to="/guests"
            className="mt-3 inline-flex items-center gap-1.5 text-[12.5px] font-medium text-primary hover:text-primary-mid"
          >
        ...
        <ul className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {guests.map((g, i) => {
            ...
            return (
              <li
                key={g.id}
                className="group relative flex items-center gap-3 rounded-2xl border border-border/60 bg-background/60 p-3 transition-all hover:-translate-y-[1px] hover:border-primary/30 hover:shadow-[0_10px_28px_-18px_rgba(17,26,19,0.35)]"
              >
