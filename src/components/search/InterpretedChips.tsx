...
const LABELS: Record<NLMissingField, string> = {
  city: "Destination",
...
export function InterpretedChips({ interpreted, missing }: Props) {
  if (!interpreted) return null;

  const chips: Array<{ key: string; label: string }> = [];
  if (interpreted.city) chips.push({ key: "city", label: interpreted.city });
  ...
  if (chips.length === 0 && (!missing || missing.length === 0)) return null;

  return (
    <div className="rounded-2xl bg-accent-pale/40 p-4 ring-1 ring-black/[0.04]">
      ...
      {chips.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {chips.map((c) => (
            <span
              key={c.key}
              className="rounded-full bg-background px-3 py-1 text-[12px] text-ink ring-1 ring-black/[0.04]"
            >
              {c.label}
            </span>
          ))}
