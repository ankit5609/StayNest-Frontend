...
import type { NLInterpretedQuery, NLMissingField, SortBy } from "@/lib/api/types";
...
const friendlyMissingNames: Record<string, string> = {
...
function MissingFields({ missing }: { missing: NLMissingField[] }) {
  const labels = missing.map((f) => friendlyMissingNames[f] ?? f);
  return (
    <div className="mr-6 rounded-2xl bg-accent-pale/40 px-3.5 py-2.5 text-[12.5px] text-ink">
...
function ConfirmSummary({ interpreted }: { interpreted?: NLInterpretedQuery }) {
  if (!interpreted) return null;
  return (
    <div className="mr-6 space-y-2.5 rounded-2xl bg-accent-pale/40 px-4 py-3 text-[12.5px] text-ink">
...
export function AssistantPanel({
  ...
  return (
    <aside className="z-20 w-full lg:sticky lg:top-[240px]">
      <div
    ...
    <header className="shrink-0 flex items-center gap-2">
        ...
        <div className="max-h-[360px] space-y-3 overflow-y-auto scroll-pt-4 pt-6 pb-4 pr-1">
          ...
          {isThinking && (
            <div className="mr-6 rounded-2xl bg-accent-pale/40 px-3.5 py-2.5 text-[13px] text-muted-foreground">
              Thinking…
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl bg-surface/50 p-4 ring-1 ring-black/[0.04]">
          ...
          <select
            ...
            className="mt-1.5 w-full appearance-none rounded-xl border border-border bg-card px-3 py-2 text-[13px] text-ink focus:border-primary focus:outline-none"
          >
            {SORT_OPTIONS.map((s) => (
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <label className="block">
              ...
              <input
                ...
                placeholder="$50"
                className="mt-1.5 w-full rounded-xl border border-border bg-card px-3 py-2 text-[13px] text-ink focus:border-primary focus:outline-none"
              />
            </label>
            <label className="block">
              <span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
          ...
          <button
            ...
            onClick={() =>
              onPriceApply(
                minLocal ? Number(minLocal) : undefined,
                maxLocal ? Number(maxLocal) : undefined,
              )
            }
            className="mt-3 w-full rounded-full bg-primary py-2.5 text-[12.5px] font-medium text-primary-foreground transition-colors hover:bg-primary-mid"
          >
            Apply
          </button>
        </div>
        ...
        <form
          ...
          className="flex items-center gap-2 rounded-full bg-accent-pale/40 pl-4 pr-1 py-1 ring-1 ring-black/[0.04] focus-within:ring-primary/40"
        >
          <input
            type="text"
            value={input}
            ...
            className="min-w-0 flex-1 bg-transparent py-2 text-[13px] text-ink placeholder:text-muted-foreground/80 focus:outline-none"
          />
          <button
            type="submit"
            aria-label="Send"
            disabled={!input.trim()}
            className="grid h-9 w-9 place-items-center rounded-full bg-primary text-primary-foreground transition-colors hover:bg-primary-mid disabled:opacity-40"
          >
        ...
        <div className="flex flex-wrap gap-2">
          {suggestions.map((s) => (
            <button
              ...
              type="button"
              onClick={() => onAsk(s)}
              className="rounded-full border border-border bg-background px-3 py-1.5 text-[11.5px] text-ink transition hover:border-primary hover:text-primary"
            >
              {s}
            </button>
          ))}
