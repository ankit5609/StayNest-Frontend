...
import { login, persistSession, type LoginResponse } from "@/lib/api/auth";
...
export function LoginForm() {
  ...
  const mutation = useMutation({
    mutationFn: login,
    onSuccess: (res: LoginResponse, variables) => {
      const roles = Array.isArray(res.roles) ? res.roles : [];
      const isManager = roles.includes("HOTEL_MANAGER");

      if (asManager && !isManager) {
        // User asked to sign in as a hotel manager but the account has no such role.
        // Do NOT persist the session — show the gate dialog.
        setGateOpen(true);
        return;
      }

      persistSession({ ...res, email: res.email ?? variables.email });
      toast.success(
    ...
    <div className="w-full max-w-[420px]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-3" noValidate>
        ...
        <div className="grid gap-2.5">
          <SocialButton icon={<GoogleIcon />} onClick={() => toast.info("Google sign-in isn't configured yet.")}>
            Continue with Google
          </SocialButton>
          <SocialButton icon={<AppleIcon />} onClick={() => toast.info("Apple sign-in isn't configured yet.")}>
            Continue with Apple
          </SocialButton>
        </div>
      </form>
