...
export function RefundsTab({ hotelId }: { hotelId?: number }) {
  const { data, isLoading } = useRefundPending();
  const settleMut = useSettleRefund();

  const items = (data ?? []).filter(
    (b) => hotelId == null || b.hotel?.id === hotelId,
  );
...
function RefundCard({
  ...
  return (
    <article className="space-y-3 rounded-2xl border border-border/60 bg-white p-5">
