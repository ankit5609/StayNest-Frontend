...
export function BookingsTab({ hotelId }: { hotelId: number }) {
...
function BookingRow({ booking: b }: { booking: BookingDto }) {
  const statusClass =
    STATUS_STYLES[b.bookingStatus as BookingStatus] ?? "bg-surface text-ink";
  return (
    <tr className="border-t border-border/60">
      ...
      <td className="px-4 py-3">
        {b.guests?.length ? (
          <div className="flex flex-wrap gap-1">
            {b.guests.slice(0, 3).map((g, i) => (
              <span
                key={i}
                className="rounded-full bg-surface px-2 py-0.5 text-[11.5px] text-ink"
              >
                {g.name}
              </span>
            ))}
      ...
      <td className="px-4 py-3">
        <span
          className={`rounded-full px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.1em] ${statusClass}`}
        >
          {b.bookingStatus}
        </span>
      </td>
