...
import type { AdminRoomDto } from "@/lib/api/admin-types";

export function RoomsTab({ hotelId }: { hotelId: number }) {
  ...
  const [editing, setEditing] = useState<AdminRoomDto | null>(null);
  const [toDelete, setToDelete] = useState<AdminRoomDto | null>(null);
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {rooms.map((room) => (
          <article
            key={room.id}
            className="space-y-4 rounded-2xl border border-border/60 bg-white p-5"
          >
            <header className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="rounded-lg p-1.5 text-muted-foreground hover:bg-surface hover:text-primary"
            ...
            {room.amenities.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {room.amenities.map((a) => (
                  <span
                    key={a}
                    className="rounded-full border border-border/60 bg-surface px-2.5 py-1 text-[11.5px] text-ink"
                  >
                    {a}
                  </span>
                ))}
            ...
            <PhotoUploader
              label="Photos"
              photos={room.photos}
              onUpload={(file) => uploadMut.mutateAsync({ roomId: room.id, file })}
            />
          </article>
        ))}
      </div>
      ...
      <ConfirmDeleteDialog
        ...
        description="This removes the room type and its inventory calendar. Existing bookings are unaffected."
